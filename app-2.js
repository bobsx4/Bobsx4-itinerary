(() => {
  ...
  function openItineraryDialog(dayId) {
    ...
    $("#itinerary-editor-status").textContent = "Local edits stay on this device and are included in backups.";
  ...
  function saveItineraryDay(event) {
    ...
    Object.entries(ITINERARY_FIELDS).forEach(([field, definition]) => {
      ...
      const published = normalizeEditableValue(definition.type, publishedDay[field]);
      if (!valuesEqual(value, published)) fields[field] = { value, base: clone(published) };
    });
    const adventureState = getAdventureState();
  ...
  function bindEvents() {
    ...
    document.addEventListener("input", (event) => {
      ...
      if (itineraryField) {
        updateItineraryFieldMarker(itineraryField[0]);
        return;
      }
