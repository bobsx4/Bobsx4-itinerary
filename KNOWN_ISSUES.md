...
Profiles, counters, journals, badges, hotel confirmations, and notes remain local to the current phone, iPad, or browser. A backup copies state but does not merge two family members' entries.
...
RC2 does not call an undocumented GasBuddy interface. A later release may provide open-in-GasBuddy/search links, vehicle range, and refuelling-area guidance while reserving direct live-price integration for an approved interface.

### 6. One visible Adventure

The data model supports an Adventures array, but RC2 does not expose a selector or builder.

### 7. Changing a profile's experience after use

A profile can be switched between Navigator and Explorer in Settings. Existing entries remain stored, but each mode intentionally shows its own missions and observation list. For the cleanest scrapbook, choose a mode for each person before the trip and keep it.

### 8. Service-worker timing
